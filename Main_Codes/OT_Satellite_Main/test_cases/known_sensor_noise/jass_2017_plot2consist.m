...
ylimits = [6.60*10^6 7*10^6; -0.008 0.008; -0.008 0.006; 0.05 0.5; -0.5 -0.09; -4 8];
figure;
...
%     ax.FontWeight = 'bold';
    ...
    if i == 1
        title('OT (samples=60,repeat=10)','FontSize',18,'interpreter', 'latex','FontWeight','bold');
    end
    ax2 = subplot(7,2,2*i);
    % PLot EnKF
...
%     ax.FontWeight = 'bold';
    ...
    if i == 1
        title('EnKF (samples=60,repeat=10)','FontSize',18,'interpreter', 'latex','FontWeight','bold');
    end
    linkaxes([ax1,ax2],'xy')
end
hL = subplot(7,2,13.5);
...
print(fig, '-dpdf', './Plots/plotconsist60.pdf');