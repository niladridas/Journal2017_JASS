...
%         for s = 1:6
%             subplot(3,2,s)
%             [f,xi] = ksdensity(X_init_enkf(s,:));
%             plot(xi,f);
%             legend('Posterior at T = 1','Posterior at T = 2','Posterior at T = 3');
%             grid on
%             hold on
%         end
%         hold on
%     end
    ...
    % Estimate the R_enkf from the data
    % Error between the estimated (assumed to be the true state values) and
    % the observation
    
    error_enkf = [error_enkf;measured_output(j,:)-x_est_enkf(j,:)];
%     error_enkf(:,6) = mod(error_enkf(:,6))
...
toc
EnKF_time = toc-tic;
% matfname = strcat('x_est_enkf',num2str(samples),'.mat');
...
toc
OT_time = toc-tic;
% matfname = strcat('x_est_OT',num2str(samples),'.mat');
% save(matfname,'x_est_OT');
